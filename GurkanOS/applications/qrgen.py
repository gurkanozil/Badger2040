...
while True:
    ...
    if TOTAL_CODES > 1:
        if display.pressed(badger2040.BUTTON_UP):
            state["current_qr"] = (state["current_qr"] - 1)
    ...
    if display.pressed(badger2040.BUTTON_B) or display.pressed(badger2040.BUTTON_C):
        display.set_pen(15)
        display.clear()
        badger_os.warning(display, "To add QR codes, connect the Badger2040 to a PC, load up your IDE, and add files to /qrcodes directory.")
        time.sleep(4)
        changed = True
