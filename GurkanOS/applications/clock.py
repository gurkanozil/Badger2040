...
def draw_clock():
    ...
    d_width = display.measure_text(ymd[0:2], 1.0)
    m_width = display.measure_text(ymd[3:5], 1.0)
    m_offset = display.measure_text(ymd[0:3], 1.0)
    y_width = display.measure_text(ymd[6:10], 1.0)
    y_offset = display.measure_text(ymd[0:6], 1.0)
    ...
    if set_clock:
        display.set_pen(0)
        if cursors[cursor] == "day":
            display.line(ymd_offset, 120, ymd_offset + d_width, 120, 4)
        if cursors[cursor] == "month":
            display.line(ymd_offset + m_offset, 120, ymd_offset + m_offset + m_width, 120, 4)
        if cursors[cursor] == "year":
            display.line(ymd_offset + y_offset, 120, ymd_offset + y_offset + y_width, 120, 4)
...
if (year, month, day) == (2021, 1, 1):
    rtc.datetime((2024, 12, 01, 0, 12, 0, 0, 0))
