...
# original code showing all apps in alphabetical order
# applications = [x[:-3] for x in os.listdir("/applications") if x.endswith(".py")]

# Define the custom order for apps
custom_order = [
    "badge",  # Custom_Badge app
    "image",  # Image app
    "info",   # Info app
    "qrgen",  # QR Generator app
    "list",   # List app
    "clock",  # Clock app
#    "ebook",  # Ebook app
#    "fonts",  # Fonts app
...
# Reorder apps according to custom order
apps_in_dir = [x[:-3] for x in os.listdir("/applications") if x.endswith(".py")]
applications = [app for app in custom_order if app in apps_in_dir]

# Approximate center lines for buttons A, B and C
centers = (41, 147, 253)

MAX_PAGE = math.ceil(len(applications) / 3)
...
def render():
    ...
    max_icons = min(3, len(applications[(state["page"] * 3):]))

    for i in range(max_icons):
        x = centers[i]
        label = applications[i + (state["page"] * 3)]
        icon_label = label.replace("_", "-")
        icon = f"icons/icon-{icon_label}"
    ...
    draw_disk_usage(90)
    display.set_pen(15)
    display.text("GM_OS", 4, 4, WIDTH, 1.0)
...
def launch_example(index):
    ...
    file = applications[(state["page"] * 3) + index]
    file = f"{APP_DIR}/{file}"
